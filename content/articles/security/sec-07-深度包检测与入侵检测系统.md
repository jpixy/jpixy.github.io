...
│  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━                         │
│  • 定义：分析数据包的应用层内容，而非仅看包头                       │
│  • 能力：识别应用协议、检测恶意内容、提取元数据                     │
│  • 用途：网络监控、流量分类、安全检测、QoS                         │
│  • 是一种技术，不是产品类别                                         │
│                                                                      │
│  IDS (Intrusion Detection System) - 入侵检测系统                    │
│  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━                   │
│  • 定义：检测网络或系统中的恶意活动和违规行为                       │
│  • 模式：被动监控，检测并告警，不主动阻断                           │
│  • 部署：通常旁路部署（镜像流量）                                   │
│  • 目的：发现攻击，提供取证信息                                     │
│                                                                      │
...
│  ━━━━━━━━━━━━━━━━━━━━━━━━━━━                                        │
│                                                                      │
│     Internet ────► Firewall ────► Switch ────► Internal            │
│                                      │                               │
│                                      │ (镜像端口)                    │
│                                      ▼                               │
│                                   ┌─────┐                           │
...
│  ━━━━━━━━━━━━━━━━━━━━━━━━━                                          │
│                                                                      │
│     Internet ────► Firewall ────► IPS ────► Switch ────► Internal  │
│                                    │                                 │
│                                    └──► Drop / Alert                │
│                                                                      │
│  优点：可以实时阻断攻击                                             │
...
│     │  │   应用程序                │   │                             │
│     │  ├──────────────────────────┤   │                             │
│     │  │   HIDS Agent             │   │◄── 监控文件/进程/网络      │
│     │  ├──────────────────────────┤   │                             │
│     │  │   操作系统                │   │                             │
│     │  └──────────────────────────┘   │                             │
│     └─────────────────────────────────┘                             │
│                                                                      │
│  例如：OSSEC, Wazuh, Tripwire                                       │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
│      ▼                                                               │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ 2. 协议识别 (Protocol Identification)                        │   │
│  │    • 端口匹配（不可靠）                                      │   │
│  │    • 协议指纹匹配                                            │   │
│  │    • 统计/机器学习方法                                       │   │
│  └─────────────────────────────────────────────────────────────┘   │
...
┌─────────────────────────────────────────────────────────────────────┐
│                                                                      │
│  1. 基于签名（Signature-based / Misuse Detection）                  │
│  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━                   │
│                                                                      │
│  原理：匹配已知攻击的特征模式                                       │
│                                                                      │
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  3. 基于策略（Policy-based / Specification-based）                  │
│  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━                  │
│                                                                      │
│  原理：定义允许的行为规范，违反则告警                               │
│                                                                      │
...
│  • pass: 放行                                                       │
│  • drop: 丢弃（IPS 模式）                                           │
│  • reject: 丢弃并发送 RST/ICMP                                      │
│                                                                      │
│  Protocol：                                                          │
│  • ip, tcp, udp, icmp                                               │
│  • http, dns, tls, ssh 等应用层协议                                 │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
│                                                                      │
│  传统内核路径：                                                      │
│  网卡 → 中断 → 内核协议栈 → 用户空间 → DPI                         │
│                 ▲                                                    │
│                 └── 多次上下文切换和拷贝                            │
│                                                                      │
│  DPDK 路径：                                                         │
│  网卡 → DPDK Poll Mode Driver → 用户空间 DPI                       │
│                 ▲                                                    │
│                 └── 零拷贝、无中断、用户态直接处理                  │
│                                                                      │
│  DPDK 优势：                                                         │
...
│  • 包含 QUIC 支持                                                   │
│  • 分段哈希便于部分匹配                                             │
│  • 格式：协议_版本_SNI_cipher数量_ext数量_ALPN                      │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
