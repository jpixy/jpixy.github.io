...
| 因素类别 | 权重 | 考量项 |
|----------|------|--------|
| **延迟因素** | 50% | 到交易所匹配引擎的物理距离、网络跳数、交换机延迟、可获得的最低延迟套餐 |
| **成本因素** | 25% | 机柜租赁费用、电力费用（$/kW）、带宽费用、交叉连接费用、人员进出费用 |
| **可靠性因素** | 15% | 电力冗余（N+1, 2N）、冷却冗余、网络冗余、历史故障率 |
| **运营因素** | 10% | 7x24远程手服务、物流便利性、扩展能力、安全认证 |
...
graph TB
    ...
    Exchange -->|Cross-connect| Primary
    Exchange -->|Cross-connect| Secondary
    Primary <-->|ISL Link| Secondary
    Primary --> TS1
    Primary --> TS2
    Primary --> MD
    Secondary --> Backup
...
| 型号 | 延迟 | 端口 | 特点 | 适用场景 |
|------|------|------|------|----------|
| Arista 7130 Series | 350ns (L2 cut-through) | 48x25G + 6x100G | FPGA可编程、纳秒级时间戳 | 顶级HFT |
| Cisco Nexus 3548 | 250ns (L2) | 48x10G | 成熟稳定 | 通用低延迟 |
| Mellanox SN2100 | 300ns | 16x100G | 高带宽密度 | 高吞吐场景 |
...
| 层级 | 名称 | 安全措施 |
|------|------|----------|
| 第1层 | 建筑外围 | 围栏/围墙、CCTV监控、车辆检查、访客登记 |
| 第2层 | 建筑入口 | 门禁卡、生物识别（可选）、安保人员、金属探测器 |
| 第3层 | 数据中心大厅 | 双因素认证、人员陪同制度、CCTV全覆盖、防尾随门 |
| 第4层 | 机柜/笼位 | 独立机柜锁、智能门锁（审计日志）、运动传感器、机柜内监控 |
