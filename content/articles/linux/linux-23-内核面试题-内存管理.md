...
| 0x0000_8000_0000_0000 ~ 0xFFFF_7FFF_FFFF_FFFF | 非规范地址空洞 (不可访问) | - |
...
| 地址范围 | 区域 | 说明 |
|----------|------|------|
| 0xFFFF_FFFF_FEE0_0000 ~ 0xFFFF_FFFF_FFFF_FFFF | 固定映射区 (fixmap) | 编译时确定的虚拟地址 |
| 0xFFFF_FFFF_FF60_0000 ~ 0xFFFF_FFFF_FEE0_0000 | VSYSCALL 页 | 兼容老程序 |
| 0xFFFF_FFFF_C000_0000 ~ 0xFFFF_FFFF_FF60_0000 | 模块映射区 | 内核模块加载 |
| 0xFFFF_EA00_0000_0000 ~ 0xFFFF_FFFF_C000_0000 | vmemmap 区 | struct page 数组 |
| 0xFFFF_C900_0000_0000 ~ 0xFFFF_EA00_0000_0000 | vmalloc 区 | vmalloc 分配 |
| 0xFFFF_8880_0000_0000 ~ 0xFFFF_C900_0000_0000 | 直接映射区 | 线性映射所有物理内存 |
...
| 地址范围 | 区域 | 说明 |
|----------|------|------|
| ~0x7FFF_FFFF_FFFF | 栈 (Stack) | 向下增长 |
| 随机间隙 | ASLR | 地址空间布局随机化 |
| mmap 区域 | 共享库 (.so) | 动态链接库 |
| 堆顶以上 | 堆 (Heap) | 向上增长 |
| BSS 段 | 未初始化全局变量 | |
| Data 段 | 已初始化全局变量 | |
| Text 段 | 代码段 | 从 0x0000_0040_0000 开始 |
...
| [29:21] | PMD | 9位 | 512 |
| [20:12] | PTE | 9位 | 512 |
| [11:0] | Offset | 12位 | 4096 (4KB) |

**说明：**
- 每级索引 9 位 → 512 个条目
- 每个条目 8 字节 → 每个页表页 4KB

**地址转换过程：** CR3 → PGD表 → PUD表 → PMD表 → PTE表 → 物理页
...
| 位 | 名称 | 说明 |
|-----|------|------|
| 63 | NX | 禁止执行 (No Execute) |
| 62:52 | 保留 | - |
| 51:12 | 物理页帧号 | Physical Frame Number |
| 11:9 | AVL | 可用位 |
| 8 | G | Global |
...
| 2 | U/S | User/Supervisor (0=内核态, 1=用户态可访问) |
| 1 | R/W | Read/Write (0=只读, 1=可写) |
| 0 | P | Present (页面是否在内存中) |
...
graph TB
    ...
    HIGH -->|"空闲页充足<br>直接分配"| LOW
    LOW -->|"唤醒 kswapd<br>后台回收"| MIN
    MIN -->|"直接回收<br>可能 OOM"| ZERO
```
