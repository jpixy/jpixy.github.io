...
| 偏移 | 内容 | 说明 |
|------|------|------|
| 0 | vptr | 指向 Derived 的虚函数表（8字节） |
| 8 | Base::x | 继承自 Base（4字节） |
| 12 | padding | 4字节 |
| 16 | Derived::y | 派生类成员（4字节） |
| 20 | padding | 4字节 |
...
**Derived 对象**（vptr + base_data + derived_data）→ vptr 指向 Derived 的 vtable
...
**循环引用问题**：Node A 和 Node B 互相用 shared_ptr 指向对方，引用计数永远不为 0，内存泄漏！

**解决方案**：使用 weak_ptr 打破循环，一方使用 shared_ptr，另一方使用 weak_ptr，可以正确释放。
...
| 阶段 | 内容 |
|------|------|
| 原始内存 | `char buffer[sizeof(MyClass)]` |
| placement new | `new(buffer) MyClass(42)` |
| 构造后 | MyClass 对象（value = 42） |
...
```mermaid
graph TB
    A["Object A<br/>data: [1,2,3]"] -->|拷贝| B["Object B<br/>data: [1,2,3] (新分配内存，复制数据)"]
```
...
```mermaid
graph TB
    A1["Object A<br/>data: [1,2,3]"] -->|移动| B1["Object B<br/>data: [1,2,3]"]
    A1 -.->|清空| A2["Object A<br/>data: nullptr"]
```

| 语义 | 过程 | Object A 状态 | Object B 状态 |
|------|------|--------------|--------------|
| 拷贝 | 新分配内存，复制数据 | 不变 | 独立副本 |
| 移动 | 转移所有权 | 清空 (nullptr) | 接管原数据 |
