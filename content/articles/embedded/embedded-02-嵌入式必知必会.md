...
graph TB
    subgraph 内存布局
        direction TB
        Stack["栈区 Stack ↓<br/>局部变量、函数调用上下文"]
        Unused["未使用区域 ↕"]
        Heap["堆区 Heap ↑<br/>动态分配内存"]
        BSS["BSS段<br/>未初始化全局/静态变量"]
        Data["Data段<br/>已初始化全局/静态变量"]
        Text["Text段<br/>程序代码"]
    end
...
```mermaid
graph TB
    A["应用层<br/>app_sensor_read()"] --> B["驱动层<br/>i2c_read(), spi_write()"]
    B --> C["HAL层<br/>HAL_I2C_Master_Transmit()"]
    C --> D["寄存器层<br/>I2C1->DR = data"]
    D --> E["硬件"]
```
...
```mermaid
graph TB
    HSE["HSE<br/>外部晶振"] --> PLL["PLL<br/>倍频器"]
    HSI["HSI<br/>内部RC"] --> PLL
    PLL --> SYSCLK["SYSCLK"]
    SYSCLK --> AHB["AHB"]
    AHB --> APB1["APB1"]
    ...
    LSE["LSE<br/>32.768kHz"] --> RTC["RTC"]
    LSI["LSI<br/>内部低速"] --> IWDG["IWDG"]
```
