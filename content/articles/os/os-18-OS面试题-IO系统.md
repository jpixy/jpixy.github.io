...
┌────────────────────────────────────────────────────────┐
│ 随机 I/O：                                              │
│   请求序列：100, 500, 200, 600, 300                     │
│   磁头移动：100→500→200→600→300 = 1200 柱面距离         │
│                                                        │
│ 调度后（SCAN）：                                        │
│   请求序列：100, 200, 300, 500, 600                     │
│   磁头移动：100→200→300→500→600 = 500 柱面距离          │
│                                                        │
│   性能提升：约 2.4 倍                                   │
└────────────────────────────────────────────────────────┘
```
...
│  │         按到达时间排序，用于截止时间检查                 │ │
│  │                                                       │ │
│  │  读 FIFO: [req1] → [req2] → [req3] → ...              │ │
│  │           deadline: 500ms                             │ │
│  │                                                       │ │
│  │  写 FIFO: [req1] → [req2] → [req3] → ...              │ │
│  │           deadline: 5000ms                            │ │
│  └───────────────────────────────────────────────────────┘ │
│                                                             │
└─────────────────────────────────────────────────────────────┘
...
│                                                        │
│  顺序读：150-200 MB/s                                   │
│  随机读：0.5-2 MB/s（受寻道时间限制）                    │
│  随机读 IOPS：100-200                                   │
│                                                        │
│  寻道时间主导延迟 → 需要 I/O 调度优化                    │
└────────────────────────────────────────────────────────┘
...
│  │    文件 A                文件 B         │               │
│  │  ┌───────────┐        ┌───────────┐   │               │
│  │  │offset 0─4K│        │offset 0─4K│   │               │
│  │  │  → Page 0 │        │  → Page 4 │   │               │
│  │  │offset 4K─8K│       │offset 4K─8K│  │               │
│  │  │  → Page 1 │        │  → Page 5 │   │               │
│  │  └───────────┘        └───────────┘   │               │
│  └─────────────────────────────────────────┘               │
│                                                             │
...
1. 设备插入
   ┌─────────────────────────────────────────────────────┐
   │ a. 总线检测到新设备                                  │
   │ b. 总线调用 bus->probe() 或发送 uevent               │
   │ c. udev 接收 uevent，创建 /dev 节点                  │
   │ d. 内核匹配驱动，调用 driver->probe()                │
   │ e. 驱动初始化设备                                    │
   └─────────────────────────────────────────────────────┘
...
2. 设备拔出
   ┌─────────────────────────────────────────────────────┐
   │ a. 总线检测到设备移除                                │
   │ b. 内核调用 driver->remove()                        │
   │ c. 驱动清理资源                                      │
   │ d. udev 接收 uevent，删除 /dev 节点                  │
   └─────────────────────────────────────────────────────┘
...
│  CPU                                                   │
│   │                                                    │
│   │  in al, 0x3F8    ; 从端口 0x3F8 读取               │
│   │  out 0x3F8, al   ; 写入端口 0x3F8                  │
│   │                                                    │
│   ▼                                                    │
│  ┌─────────────────┐                                  │
│  │ I/O 端口空间     │  0x0000 - 0xFFFF (64KB)         │
│  │  0x3F8: COM1    │                                  │
│  │  0x378: LPT1    │                                  │
│  └─────────────────┘                                  │
└────────────────────────────────────────────────────────┘
...
│  CPU                                                   │
│   │                                                    │
│   │  mov eax, [0xFEDC0000]  ; 读取                     │
│   │  mov [0xFEDC0000], eax  ; 写入                     │
│   │                                                    │
│   ▼                                                    │
│  ┌─────────────────────────────────────────┐          │
│  │           物理地址空间                    │          │
