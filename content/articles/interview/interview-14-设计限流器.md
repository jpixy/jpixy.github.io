...
│  请求日志:                                                                   │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │  [ 12:00:00.100, 12:00:00.250, 12:00:00.500, 12:00:00.750, ... ]    │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
│  算法流程:                                                                   │
...
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │  数据结构:                                                           │    │
│  │  KEY: rate_limiter:{resource}:{minute}                              │    │
│  │  HASH: field = 秒级时间片, value = 计数                             │    │
│  │                                                                     │    │
...
│  4. 分片                                                                     │
│     • 将热点 Key 分片，减少单点压力                                          │
│     • 例如: rate_limit:api:order:{hash % 10}                                │
│                                                                              │
│  5. Lua 脚本优化                                                             │
│     • 使用 Redis Lua 脚本减少网络往返                                        │
│     • 预加载脚本 (SCRIPT LOAD)                                               │
│                                                                              │
│  6. 连接池                                                                   │
│     • Redis 连接池复用                                                       │
...
│  3. 资源推算法                                                               │
│     • 根据资源容量反推                                                       │
│     • 例如: 数据库 1000 连接，单请求 10ms                                    │
│     • 最大 QPS = 1000 / 0.01 = 100,000                                      │
│                                                                              │
│  4. 动态调整                                                                 │
│     • 监控系统负载                                                           │
│     • 根据负载动态调整阈值                                                   │
...
│     │  • 响应头: X-RateLimit-Reset: 1704700000                        │     │
│     │                                                                  │     │
│     │  客户端根据响应头:                                               │     │
│     │  • 暂停请求                                                      │     │
│     │  • 指数退避重试                                                  │     │
...
方案 1: 配置中心联动
┌─────────────────────────────────────────────────────────────────────────────┐
│  1. HPA 扩容: 2 Pod → 4 Pod                                                │
│  2. Operator 监听 Pod 变化                                                 │
│  3. Operator 更新配置中心: 单机限流 = 1000/4 = 250 QPS                     │
│  4. 各 Pod 监听配置变化，热更新                                             │
└─────────────────────────────────────────────────────────────────────────────┘

方案 2: Sentinel 集群限流
┌─────────────────────────────────────────────────────────────────────────────┐
│  1. Token Server 维护全局令牌总量                                          │
│  2. 各 Pod 作为 Token Client                                               │
│  3. Pod 数量变化时，自动重新分配令牌                                        │
│  4. 无需手动调整配置                                                        │
└─────────────────────────────────────────────────────────────────────────────┘

方案 3: Redis 全局限流 (推荐)
┌─────────────────────────────────────────────────────────────────────────────┐
│  1. 所有 Pod 共享 Redis 限流                                               │
│  2. 全局阈值固定                                                           │
│  3. Pod 数量变化不影响限流逻辑                                              │
│  4. 天然支持动态扩缩容                                                      │
└─────────────────────────────────────────────────────────────────────────────┘
