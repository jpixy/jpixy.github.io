...
graph TB
    ...
    EA -->|UDP Multicast| NET
    EB -->|TCP| NET
    DP --> NET
    subgraph NET["网络层 (< 1μs)"]
        NIC["NIC (Solarflare/Mellanox)<br/>+ Kernel Bypass (DPDK/Onload)"]
    end
    NET --> MDP
    subgraph MDP["市场数据处理 (< 5μs)"]
        FH[Feed Handler] --> PP["协议解析<br/>(ITCH/OUCH)"]
        PP --> NM[规范化 Normalizer]
    end
    MDP --> OBM
    subgraph OBM["订单簿管理 (< 2μs)"]
        OBE["Order Book Engine<br/>(无锁数据结构，缓存优化)"]
    end
    OBM --> SE
    subgraph SE["策略引擎 (< 5μs)"]
        SG[信号生成] --> RC[风险检查] --> OG[订单生成]
    end
    SE --> OMS
    subgraph OMS["订单管理 (< 2μs)"]
        OMSS["Order Management System"]
    end
    OMS --> GW
    subgraph GW["网关层 (< 1μs)"]
        OGW["Order Gateway<br/>(协议编码 → 发送)"]
    end
    GW --> EXO[交易所]
```
...
```mermaid
graph TB
    A1[应用层] -->|~100-200ns 系统调用| B1[Socket 层]
    B1 -->|~1-5μs 协议栈处理| C1[TCP/IP 栈]
    C1 -->|~100ns 内核调度| D1[设备驱动]
    D1 -->|~2-10μs 中断处理| E1[NIC]
```
...
```mermaid
graph TB
    A2[应用层] -->|直接访问（无系统调用）| B2["用户态驱动 (PMD)"]
    B2 -->|轮询（无中断）| C2[NIC]
```
...
```mermaid
graph TB
    subgraph BUDGET["端到端延迟 < 20μs"]
        A["网络接收<br/>< 1μs"] --> B["解析处理<br/>< 2μs"]
        B --> C["订单簿更新<br/>< 2μs"]
        C --> D["策略决策<br/>< 5μs"]
        D --> E["风控检查<br/>< 1μs"]
        E --> F["订单编码<br/>< 1μs"]
...
graph TB
    subgraph PDC["主数据中心"]
        P["主交易节点<br/>(Active)"] <--> S["备交易节点<br/>(Standby)"]
        S <--> A["仲裁节点<br/>(Arbiter)"]
        P --> SYNC["状态同步层 (< 10μs)<br/>共享内存 / RDMA / Replicated Log"]
        S --> SYNC
    end
    PDC -->|异地复制| DR
    subgraph DR["灾备数据中心"]
...
graph TB
    subgraph TS["交易系统"]
        MC["内置指标采集器 (无锁、无分配)<br/>- 延迟直方图<br/>- 吞吐量计数器<br/>- 错误计数器"]
    end
    TS -->|共享内存 / UDP| MA
    subgraph MA["监控代理 独立进程"]
        AG[指标聚合]
        AE[告警评估]
        LC[日志收集]
    end
