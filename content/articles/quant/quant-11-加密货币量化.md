...
graph TB
    ...
    量化机遇 --> 核心原则["⚠️ 不要把所有资金放在交易所<br/>使用可承受损失的资金"]
```
...
**原理：**
同一币种在不同交易所价格可能不同，在低价所买入，在高价所卖出
...
```mermaid
graph TB
    subgraph 风险应对["加密货币特有风险及应对"]
        subgraph 交易所风险
            J1["风险：交易所破产（如 FTX）、被黑"]
            J2["应对：分散在多个交易所"]
            J3["不在交易所存大量资金"]
            J4["使用冷钱包存储"]
        end
        ...
        subgraph API风险["API 风险"]
            A1["风险：API 故障、延迟、被限流"]
            A2["应对：错误处理、重试机制、监控系统"]
        end
    end
```
...
```mermaid
graph TB
    subgraph 入门路径["加密货币量化入门路径"]
        subgraph 阶段1["阶段1：学习（1-2个月）"]
            X1["了解加密货币基础"]
            X2["熟悉交易所操作"]
            X3["学习 ccxt 库"]
            X4["获取和处理数据"]
        end
        subgraph 阶段2["阶段2：回测（1-2个月）"]
            H1["获取历史数据"]
            H2["回测简单策略"]
            H3["理解加密货币市场特点"]
        end
        subgraph 阶段3["阶段3：模拟（1-2个月）"]
            M1["使用测试网/模拟盘"]
            M2["测试自动化系统"]
            M3["验证执行逻辑"]
        end
        subgraph 阶段4["阶段4：实盘（持续）"]
            S1["小资金开始"]
            S2["简单策略开始"]
            S3["逐步增加复杂度和资金"]
        end
...
graph TB
    ...
    核心要点 --> 结论["加密货币有机会，但风险更大<br/>敬畏市场，保护本金"]
```
