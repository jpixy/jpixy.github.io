...
flowchart TD
    LOOP[主循环<br/>遍历每个时间点]
    DATA[数据模块<br/>提供当前bar的市场数据]
    STRATEGY[策略模块<br/>根据数据生成交易信号]
    RISK[风控模块<br/>检查风控规则，决定仓位]
    EXEC[执行模块<br/>模拟订单执行，考虑滑点]
    ACCOUNT[账户模块<br/>更新持仓、资金、净值]
    
    LOOP --> DATA
    DATA -->|Market Event| STRATEGY
    STRATEGY -->|Signal Event| RISK
    RISK -->|Order Event| EXEC
    EXEC -->|Fill Event| ACCOUNT
```
...
**示例：**
2010 年买入"便宜"股票，其中一些后来退市归零，只用当前存在的股票回测会忽略这些亏损
