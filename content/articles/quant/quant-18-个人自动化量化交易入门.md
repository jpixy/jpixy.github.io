...
**结论**："不要在自己没有优势的战场上战斗"——个人应该专注中低频，利用自己的灵活性优势。
...
如果控制仓位在 30%：实际杠杆 = 名义杠杆 × 30% ≈ 3倍
- 价格波动 10%，账户波动约 30%
- 与股票配资 3 倍相当
- 风险可接受

如果控制仓位在 20%：实际杠杆 ≈ 2倍，风险与融资融券相当。
...
**所以：** 在中国做日内/秒级交易完全可行，只要你交易期货或可转债。
...
1. 在期货公司开户
2. 向期货公司申请程序化交易权限
3. 获取：BrokerID、交易服务器地址、行情服务器地址、授权码（AuthCode）、AppID
4. 下载 CTP 官方 SDK
5. 开发和测试
...
**结论：** 中国期货市场是成熟的、可交易的市场。量化交易完全可行，策略有效性存在。关键是选择合适的品种和策略。
...
衡量"风险调整后收益"的指标，即：每承担一单位风险，能获得多少超额收益。

**计算公式：**

$$夏普比率 = \frac{策略收益率 - 无风险利率}{策略收益率的标准差} = \frac{E[R] - R_f}{\sigma}$$
...
假设：策略年化收益 20%、无风险利率 3%、收益波动率 15%

夏普比率 = (20% - 3%) / 15% = **1.13**（良好的策略）
...
策略在历史数据上表现优异，但在新数据上表现很差。本质：学到了噪音，而非真正的规律。

---

**过拟合的常见原因：**

1. **参数过多**：策略有太多可调参数，每个参数都"恰好"适应历史数据，参数组合数量爆炸
2. **数据挖掘（Data Snooping）**：反复测试直到找到"有效"的参数，实际上是随机找到的巧合
3. **样本量太小**：用太短的数据回测，统计意义不足
4. **未来函数**：使用了回测时无法获得的信息（如用收盘价在开盘时下单）
...
graph TB
    ...
    核心要点 --> 结论["量化交易不是快速致富的捷径<br/>而是用科学方法管理投资的工具<br/>找准定位，长期主义"]
```
