...
│  AI/ML 训练与推理                                                    │
│  ━━━━━━━━━━━━━━━━━                                                  │
│  • PyTorch、TensorFlow 底层依赖 CUDA                                │
│  • 大模型训练：GPT、LLaMA、Stable Diffusion                         │
│  • 推理加速：TensorRT、vLLM                                         │
│                                                                      │
│  科学计算与 HPC                                                      │
│  ━━━━━━━━━━━━━━━━                                                   │
...
│  ┌─────────────────────────────────────────────┐                    │
│  │  ┌───────┐ ┌───────┐ ┌───────┐ ┌───────┐   │                    │
│  │  │ Core  │ │ Core  │ │ Core  │ │ Core  │   │  4-64 个强核心     │
│  │  │ ALU   │ │ ALU   │ │ ALU   │ │ ALU   │   │                    │
│  │  │ L1/L2 │ │ L1/L2 │ │ L1/L2 │ │ L1/L2 │   │  大缓存           │
│  │  └───────┘ └───────┘ └───────┘ └───────┘   │                    │
│  │  ┌─────────────────────────────────────┐   │                    │
│  │  │          大容量 L3 Cache            │   │  复杂分支预测     │
│  │  └─────────────────────────────────────┘   │                    │
...
│  ┌─────────────────────────────────────────────┐                    │
│  │  ┌─┐┌─┐┌─┐┌─┐┌─┐┌─┐┌─┐┌─┐ ... ┌─┐┌─┐┌─┐   │                    │
│  │  │C││C││C││C││C││C││C││C│     │C││C││C│   │  数千个小核心      │
│  │  └─┘└─┘└─┘└─┘└─┘└─┘└─┘└─┘     └─┘└─┘└─┘   │                    │
│  │  ┌─┐┌─┐┌─┐┌─┐┌─┐┌─┐┌─┐┌─┐ ... ┌─┐┌─┐┌─┐   │  小缓存           │
│  │  │C││C││C││C││C││C││C││C│     │C││C││C│   │                    │
│  │  └─┘└─┘└─┘└─┘└─┘└─┘└─┘└─┘     └─┘└─┘└─┘   │  简单控制         │
│  └─────────────────────────────────────────────┘                    │
│  特点：大规模并行、高吞吐量、适合数据并行任务                       │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
...
│  ━━━━━━━━━━━━━━━                ━━━━━━━━━━━━━                        │
│  • 最多 1024 个线程              • 最小执行单位                      │
│  • 共享 Shared Memory           • 有唯一的 threadIdx                 │
│  • 可以同步 (__syncthreads)     • 32 个线程组成 Warp                 │
│  • 被调度到一个 SM              • Warp 内 SIMT 执行                  │
│                                                                      │
│  索引计算：                                                          │
│  int idx = blockIdx.x * blockDim.x + threadIdx.x;                   │
│                                                                      │
│  例：gridDim=(4,2), blockDim=(256,1)                                │
│      总线程数 = 4 * 2 * 256 = 2048                                  │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
┌─────────────────────────────────────────────────────────────────────┐
│                                                                      │
│  共享内存被分成 32 个 Banks（每个 4 字节）                          │
│  同一 Warp 的线程访问同一 Bank 会产生冲突                           │
│                                                                      │
│  Bank 分布：                                                         │
...
│  • 共享内存：100KB                                                  │
│                                                                      │
│  如果 Kernel 每线程用 64 个寄存器：                                 │
│  64K / 64 = 1024 线程 = 32 Warp                                     │
...
┌─────────────────────────────────────────────────────────────────────┐
│                                                                      │
│  GPU 以 128 字节为单位访问 Global Memory                            │
│  最优情况：Warp 内连续线程访问连续地址                              │
│                                                                      │
│  合并访问（好）：                                                    │
│  ━━━━━━━━━━━━━━━                                                    │
│  Thread 0 → addr[0]                                                 │
│  Thread 1 → addr[1]       →  1 次 128B 事务                         │
│  Thread 2 → addr[2]                                                 │
│  ...                                                                │
│                                                                      │
│  非合并访问（差）：                                                  │
│  ━━━━━━━━━━━━━━━━━                                                  │
│  Thread 0 → addr[0]                                                 │
│  Thread 1 → addr[stride]  →  32 次 128B 事务（stride 大）           │
...
│  □ 安装 CUDA Toolkit                                               │
│  □ 编写第一个向量加法 Kernel                                       │
│  □ 理解线程层次（Grid/Block/Thread）                               │
│  □ 掌握基本内存管理（malloc/memcpy/free）                          │
│                                                                      │
│  阶段 2：核心概念（4-6 周）                                         │
│  ━━━━━━━━━━━━━━━━━━━━━━━━                                           │
│  □ 共享内存使用                                                    │
...
│  官方资源                                                            │
│  ━━━━━━━━                                                           │
│  • CUDA C Programming Guide（必读）                                 │
│  • CUDA Best Practices Guide（优化必读）                            │
...
│  在线课程                                                            │
│  ━━━━━━━━                                                           │
│  • Coursera: "CUDA Programming" by NVIDIA                           │
...
│  问题 1：Kernel 不执行                                               │
│  ━━━━━━━━━━━━━━━━━━━━━                                              │
│  • 检查 GPU 是否正确识别：nvidia-smi                                │
│  • 检查 Kernel 参数是否正确                                         │
│  • 检查编译架构是否匹配：-arch=sm_XX                                │
│  • 检查错误：cudaGetLastError()                                     │
│                                                                      │
│  问题 2：结果全是 0 或垃圾值                                        │
│  ━━━━━━━━━━━━━━━━━━━━━━━━━                                          │
│  • 检查 cudaMemcpy 方向是否正确                                     │
│  • 检查是否等待 GPU 完成：cudaDeviceSynchronize()                   │
│  • 检查数组越界                                                     │
│                                                                      │
│  问题 3：illegal memory access                                      │
│  ━━━━━━━━━━━━━━━━━━━━━━━━━━━                                        │
...
│  • 减少 Host-Device 数据传输                                        │
│                                                                      │
│  问题 5：共享内存 out of range                                      │
│  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━                                       │
│  • 检查 Block 大小与共享内存大小匹配                                │
│  • 动态共享内存需要正确传递大小                                     │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
