...
| **为什么计算图是「先乘后加再激活」？能打乱顺序吗？** | I (1.2 追问) | 线性可合并→激活打破线性→深度才有意义。 |
...
```
为什么要"先乘后加"？   → 线性变换：提取特征的基本操作
为什么要"激活"？       → 线性只能画直线，但现实是曲线 → 激活引入非线性 → 模型能画曲线
为什么要"多层"？       → 一层只能画简单曲线 → 多层可以画任意复杂曲线
为什么不能去掉激活？   → 没有激活 → 100层 = 1层 → 只能画直线 → 无法处理任何复杂任务
```
