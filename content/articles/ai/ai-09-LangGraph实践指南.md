...
| 概念 | 说明 |
|------|------|
| **State（状态）** | TypedDict 定义状态结构，状态在节点间传递和更新 |
| **Node（节点）** | 一个普通函数，接收 state，返回 state 更新，返回值自动合并到 state |
| **Graph（图）** | `add_node` 添加节点，`set_entry_point` 设置入口，`add_edge` 连接节点 |
| **Compile & Invoke** | `compile()` 编译成可执行的 app，`invoke()` 运行图 |
...
```mermaid
graph TB
    U["用户: 北京天气怎么样？"] --> A1["Agent: 我需要搜索北京天气<br/>→ 调用 search(北京天气)"]
    A1 --> T["Tools: 执行搜索，返回结果"]
    T --> A2["Agent: 根据搜索结果，北京今天晴，25度...<br/>→ 不需要更多工具，直接回答"]
    A2 --> E["END: 返回最终答案"]
```
...
```mermaid
graph TB
    START[START] --> P[prepare]
    P --> S[sensitive_action]
    S --> W["[暂停等待]"]
    W --> H[人工审批]
    H --> S
    S --> END[END]
