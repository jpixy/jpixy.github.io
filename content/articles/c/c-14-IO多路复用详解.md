...
| fd_set (1024 bits = 128 bytes) | 0 | 1 | 2 | 3 | 4 | 5 | 6 | 7 | ... | 1023 |
|--------------------------------|---|---|---|---|---|---|---|---|-----|------|
| 监控 fd 0, 3, 5 | 1 | 0 | 0 | 1 | 0 | 1 | 0 | 0 | ... | 0 |
...
graph TB
    subgraph "共享内存区域"
        subgraph "Submission Queue"
            SQH["SQ head"] --> SQE0["SQE 0 (用户写入)"]
            SQT["SQ tail"] --> SQE1["SQE 1"]
        end
        
        SQE1 --> |内核消费| EXEC["执行 IO"]
        
        EXEC --> |内核写入| CQE0
        
        subgraph "Completion Queue"
            CQH["CQ head"] --> CQE0["CQE 0 (用户读取)"]
            CQT["CQ tail"] --> CQE1["CQE 1"]
        end
    end
```
