...
**Bitmap 表示**：每个比特代表一个 IP 地址

| bit 0 | bit 1 | bit 2 | bit 3 | bit 4 | bit 5 | bit 6 | bit 7 |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 0 | 1 | 0 | 0 | 1 | 0 | 1 | 0 |
| IP=0 正常 | IP=1 封禁 | IP=2 正常 | IP=3 正常 | IP=4 封禁 | IP=5 正常 | IP=6 封禁 | IP=7 正常 |
...
Radix Tree = 压缩的前缀树 (Trie)，相同前缀合并为一个节点，节省空间
...
| 同步类型 | 频率 | 方式 | 目的 |
|---------|------|------|------|
| **全量同步** | 每天 1 次 (凌晨低峰期) | 拉取完整封禁列表，重建数据结构 | 纠正累积误差，保证最终一致 |
| **增量同步** | 每 5 分钟 | 只拉取变化的 IP (新增封禁 + 解除封禁) | 快速响应政策变化 |
| **实时推送** (可选) | 秒级生效 | 政府 API 支持 Webhook 回调 | 需政府 API 支持 |
...
| TTL 设置 | 优点 | 缺点 |
|---------|------|------|
| 太短 (如 1秒) | 封禁生效快 | 缓存命中率低，后端压力大 |
| 太长 (如 1小时) | 缓存命中率高，性能好 | 解封延迟长，用户体验差 |
...
graph TB
    Query["所有查询"] --> Ptr["指针切换"]
    Ptr --> Active["Active Bitmap (v1)"]
    Ptr -.->|原子切换| Standby["Standby Bitmap (v2 加载中)"]
    Load["后台加载新版本"] --> Standby
```
...
| 场景 | 触发条件 | 策略 | 告警/权限 |
|------|---------|------|----------|
| **政府 API 不可用** | 连续 3 次同步失败 | 使用本地最新快照继续服务 | P1 级别告警，通知 On-Call；记录审计日志 |
| **边缘节点数据损坏** | checksum 校验失败 | 自动回滚到上一版本 | P2 级别告警 |
| **大规模误封 (紧急回滚)** | 封禁率突然飙升或客诉激增 | 一键关闭过滤，全部放行 | 需要 SRE 负责人审批 |
...
| 阶段 | 内容 |
|------|------|
| **Week 1-2: 基础设施** | 对接政府 API，确认接口规格；搭建同步服务原型；确定数据结构 (Bitmap/Radix Tree) |
| **Week 3-4: 核心开发** | 实现同步服务 (全量 + 增量)；实现边缘节点过滤模块；单元测试 + 集成测试 |
| **Week 5: 暗上线 (Shadow Mode)** | 部署到生产环境；只记录日志，不实际封禁；验证命中率、误判率、延迟 |
| **Week 6: 灰度上线** | Day 1: 1% 流量 → Day 2: 5% → Day 3: 20% → Day 4: 50%；每阶段观察 24 小时 |
| **Week 7: 全量上线** | 100% 流量开启封禁；7×24 值班待命；准备紧急回滚方案 |
| **Week 8: 缓冲期** | 处理遗留问题；性能调优；文档完善 |
...
| 场景 | 问题 | 处理 |
|------|------|------|
| **NAT/代理** | 多用户共享同一出口 IP，封禁一个 IP 可能影响大量正常用户 | 无解 (法律要求)，但可记录影响范围 |
| **CDN/反向代理** | 请求经过 CDN，看不到真实 IP | 使用 X-Forwarded-For，注意伪造风险 |
| **移动网络** | 运营商 IP 池动态分配，IP 解封后可能分配给其他用户 | 依赖政府列表更新 |
| **内部流量** | 内部服务调用也会被过滤 | 内部 IP 段加入白名单 |
| **VPN/Tor 出口节点** | 用户使用 VPN/Tor 绕过封禁 | 维护已知 VPN/Tor 出口 IP 列表，可选封禁；注意可能误封正常 VPN 用户 |
| **云服务商 IP** | 攻击者使用云服务器，IP 频繁更换 | 依赖政府列表更新，可考虑额外的行为分析 |
| **双栈网络** | 同一用户可能同时有 IPv4 和 IPv6 地址 | 两个地址都需要检查，任一封禁即拒绝；注意 IPv4-mapped IPv6 (::ffff:1.2.3.4) 需特殊处理 |
| **Anycast** | 用户可能被路由到不同的边缘节点 | 确保所有节点数据同步，版本一致 |
| **IP 欺骗 (Spoofing)** | 攻击者伪造源 IP | TCP 握手天然防护，UDP 需额外验证 |
...
| 层级 | 位置 | 优点 | 缺点 | 适用场景 |
|------|------|------|------|---------|
| **Layer 3/4** (网络层/传输层) | 防火墙、负载均衡器、eBPF/XDP | 延迟极低 (<1μs)，CPU 开销小 | 无法获取 HTTP 头信息 | 大规模封禁，性能敏感场景 |
| **Layer 7** (应用层) | Nginx、应用代码 | 可获取 X-Forwarded-For、灵活定制响应 | 延迟较高，资源消耗大 | 需要识别真实 IP (CDN 后) |
...
```mermaid
graph TB
    NIC["NIC"] --> Kernel["内核协议栈"] --> TCP["TCP握手"] --> App["App处理"] --> Filter["判断封禁"]
```

**XDP 路径**：
```mermaid
graph TB
    NIC2["NIC"] --> XDP["XDP程序"] --> Result["DROP (封禁) 或 PASS (放行)"]
```
...
```mermaid
graph TB
    IP["IP"] --> BF{"Bloom Filter"}
    BF -->|不存在| Pass1["放行 (100%确定)"]
    BF -->|可能存在| Redis["查询 Redis"]
    Redis -->|存在| Block["封禁 (真阳性)"]
    Redis -->|不存在| Pass2["放行 (假阳性)"]
```
...
| Hash 函数数量 | 7 |
| Bloom Filter 层结束率 | 99% (正常 IP 直接放行) |
| 假阳性需查询 Redis | 1% |
...
| 方案 | 说明 | 优点 | 缺点 |
|------|------|------|------|
| **方案 1: 连接时检查** (推荐) | 只在连接建立时检查 IP，已建立连接不受新封禁影响 | 实现简单，用户体验好 | 存在合规窗口期 |
| **方案 2: 定期重检** | 每 N 分钟重新检查活跃连接的 IP，发现封禁后主动断开连接 | 合规性更好 | 增加系统复杂度 |
| **方案 3: 事件驱动** | 封禁列表更新时，推送到连接管理器，主动断开匹配的连接 | 实时性最好 | 架构复杂 |
...
**请求链路**：`User (1.2.3.4) → CDN (5.6.7.8) → LB (10.0.0.1) → App`
...
| 451 Unavailable For Legal Reasons | 因法律原因不可用 (RFC 7725)，推荐用于政府合规场景 |
...
graph TB
    subgraph Country_X
        GovX["Gov API X"] --> SyncX["Sync X"] --> BitmapX["Bitmap X"]
    end
    subgraph Country_Y
        GovY["Gov API Y"] --> SyncY["Sync Y"] --> BitmapY["Bitmap Y"]
    end
    subgraph Country_Z
        GovZ["Gov API Z"] --> SyncZ["Sync Z"] --> BitmapZ["Bitmap Z"]
    end
```
...
- 措施: 传输加密 (TLS 1.3)、存储加密 (S3 SSE-S3 或 SSE-KMS)、访问控制 (IAM 最小权限)、审计日志
...
| 测试类型 | 测试内容 |
|---------|---------|
| **单元测试** | Bitmap 位操作正确性、IP 解析和转换、CIDR 匹配逻辑、缓存 TTL 过期 |
| **集成测试** | 同步服务与 Mock 政府 API、边缘节点数据更新、多节点一致性 |
| **性能测试** | 查询延迟 (目标: P99 < 1ms)、吞吐量 (目标: 100万+ QPS)、同步耗时 (10亿 IP 全量同步) |
| **混沌测试** | 政府 API 不可用、Redis 宕机、网络分区、数据损坏 |
| **影子测试** | 复制生产流量到测试环境、对比过滤结果、不影响真实用户 |
| **回归测试** | 已知封禁 IP 列表验证、边界 IP 测试、IPv4-mapped IPv6 地址 |

**边界用例完整清单**：
- 空封禁列表 / 全量封禁 (43亿 IPv4 全封) / 单 IP 封禁/解封循环
- CIDR /0 (全网段) / CIDR /32 (单 IP) / 重叠 CIDR
- 私有 IP 段 / 环回地址 / 多播地址 / 链路本地地址
...
| 方案 | 组件 | 成本 |
|------|------|------|
| **方案 A: Redis SET (小规模)** | Redis Cluster (3主3从) + 同步服务 | ~$2,200/月 |
| **方案 B: Bitmap (大规模 IPv4)** | S3 存储 + CDN 分发 + 同步服务 | ~$710/月 |
| **方案 C: Radix Tree (IPv6)** | S3 存储 + CDN 分发 + 边缘节点内存 | ~$7,100/月 |
...
| SLO 类型 | SLI | SLO 目标 |
|---------|-----|---------|
| **可用性** | 成功处理的请求数 / 总请求数 | 99.99% (每月最多 4.3 分钟不可用)，错误预算 0.01% |
| **延迟** | IP 检查延迟 | P50 < 100μs, P99 < 1ms, P99.9 < 10ms |
| **数据新鲜度** | 最后成功同步时间 | 同步延迟 < 10 分钟 (99.9%)，全量同步每 24 小时至少 1 次 |
| **准确性** | 假阴性率/假阳性率 | 假阴性率 (漏封): 0%，假阳性率 (误封): < 0.001% |
| **一致性** | 版本一致的边缘节点比例 | > 99% 节点版本一致 |
...
3. 抽样检查被封禁请求 (是否有误封迹象)
4. 若确认误封: a) 回滚到上一版本 b) 或启用紧急开关暂停过滤
5. 通知产品/法务，评估影响
...
| Flag | 说明 | 默认值/选项 | 用途 |
|------|------|-------------|------|
| `ip_filter.enabled` | 总开关，控制是否启用 IP 过滤 | true (生产), false (开发) | 紧急回滚 |
| `ip_filter.mode` | 运行模式 | "enforce" / "shadow" / "log_only" | enforce=真实封禁, shadow=只记日志, log_only=只记录匹配 |
| `ip_filter.rollout_percentage` | 灰度比例 (0-100) | 0-100 | 渐进式上线, 实现: `hash(request_id) % 100 < percentage` |
| `ip_filter.whitelist_enabled` | 是否启用白名单 | true/false | 豁免特定 IP (合作伙伴、内部服务) |
| `ip_filter.fail_mode` | 出错时行为 | "open" / "closed" | open=放行 (可用性优先), closed=封禁 (合规优先, 默认) |
...
| 错误 | 问题 | 正解 |
|------|------|------|
| 每次请求都调用政府 API | 延迟高、成本高、单点故障 | 本地缓存/同步，批量拉取 |
| 使用 HashSet 存储 10 亿 IP | 内存占用 40+ GB，Redis 无法承受 | Bitmap (512MB) 或 Bloom Filter |
| IPv6 也用 Bitmap | 2^128 个地址，无法存储 | Radix Tree 或分层 HashMap |
| 一次性全量上线 | 无法验证正确性，出问题影响全量用户 | 暗上线 → 灰度 → 全量 |
| 忽略 X-Forwarded-For 伪造风险 | 攻击者可伪造 IP 绕过封禁 | 只信任可信代理，从右向左解析 |
| 没有降级策略 | 政府 API 挂了，系统无法工作 | 本地快照 + 离线模式 |
| 忽略合规/审计需求 | 无法证明系统正确执行了封禁 | 完整审计日志 + 可追溯 |
| 只考虑技术不考虑时间 | 方案完美但 2 个月无法完成 | MVP 优先，迭代优化 |
| 封禁后返回 200 OK | 用户不知道被封禁，体验差 | 返回 403/451 + 友好错误信息 |
| 没有紧急回滚机制 | 大规模误封时无法快速恢复 | Feature Flag 一键关闭 |
...
graph TB
    ...
    Q2 -->|"< 100万"| A1[Redis SET + 本地 LRU]
    Q2 -->|"100万-1亿"| A2[Bloom Filter + Redis]
    Q2 -->|"> 1亿"| A3[Bitmap 512MB]
    
    Q3 -->|主要是 CIDR| A4[Radix Tree<br/>天然支持前缀匹配]
    Q3 -->|主要是单 IP| A5[分层 HashMap<br/>按前缀分层]
    ...
    Q4 -->|秒级| B1[推送机制<br/>Kafka/WebSocket]
    Q4 -->|分钟级| B2[增量轮询<br/>每 5 分钟]
    Q4 -->|小时级| B3[全量同步即可]
    ...
    Q5 -->|"< 10万"| C1[单层架构足够]
    Q5 -->|"10万-100万"| C2[本地缓存 + 分布式存储]
    Q5 -->|"> 100万"| C3[边缘节点本地化 + eBPF 加速]
```
