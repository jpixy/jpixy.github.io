...
│  │                    召回层 (Recall/Matching)                          │    │
│  │                                                                      │    │
│  │  目标: 从海量物品(千万/亿级)中快速筛选候选集(千级)                    │    │
│  │  延迟要求: < 50ms                                                    │    │
│  │  方法: 多路召回 (协同过滤、向量召回、规则召回等)                      │    │
│  │                                                                      │    │
│  └────────────────────────────────┬────────────────────────────────────┘    │
│                                   │ ~1000 候选                              │
│                                   ▼                                         │
...
│  │                    粗排层 (Pre-Ranking)                              │    │
│  │                                                                      │    │
│  │  目标: 使用轻量模型快速过滤，减少精排压力                             │    │
│  │  延迟要求: < 20ms                                                    │    │
│  │  方法: 简化版排序模型 (双塔、轻量级 DNN)                              │    │
│  │                                                                      │    │
│  └────────────────────────────────┬────────────────────────────────────┘    │
│                                   │ ~200 候选                               │
│                                   ▼                                         │
...
│  │                    精排层 (Ranking)                                  │    │
│  │                                                                      │    │
│  │  目标: 使用复杂模型精确预估，最大化目标指标                           │    │
│  │  延迟要求: < 50ms                                                    │    │
│  │  方法: 复杂 DNN (Wide&Deep, DeepFM, DIN, DIEN 等)                    │    │
│  │                                                                      │    │
│  └────────────────────────────────┬────────────────────────────────────┘    │
│                                   │ ~50 候选 (已排序)                       │
│                                   ▼                                         │
...
│  │                    重排层 (Re-Ranking)                               │    │
│  │                                                                      │    │
│  │  目标: 业务规则、多样性、去重、打散、广告混排                         │    │
│  │  延迟要求: < 10ms                                                    │    │
│  │  方法: 规则引擎、多样性算法 (MMR, DPP)                                │    │
│  │                                                                      │    │
│  └────────────────────────────────┬────────────────────────────────────┘    │
│                                   │ ~10-20 结果                             │
│                                   ▼                                         │
...
│  │                      离线层 (Offline)                                │    │
│  │                                                                      │    │
│  │  • 模型训练 (Spark/Flink + TensorFlow/PyTorch)                       │    │
│  │  • 特征工程 (离线特征计算)                                           │    │
│  │  • 召回索引构建 (Faiss/Milvus)                                       │    │
...
│         ▼           ▼           ▼           ▼           ▼                   │
│  ┌───────────┐ ┌───────────┐ ┌───────────┐ ┌───────────┐ ┌───────────┐     │
│  │ 协同过滤  │ │ 向量召回  │ │ 热门召回  │ │ 标签召回  │ │ 图召回    │     │
│  │ (ItemCF)  │ │ (双塔)    │ │ (热榜)    │ │ (兴趣)    │ │ (GraphSage)│    │
│  │           │ │           │ │           │ │           │ │           │     │
│  │ 200条     │ │ 300条     │ │ 100条     │ │ 200条     │ │ 200条     │     │
│  └─────┬─────┘ └─────┬─────┘ └─────┬─────┘ └─────┬─────┘ └─────┬─────┘     │
│        │             │             │             │             │            │
│        └─────────────┴─────────────┴─────────────┴─────────────┘            │
│                                    │                                        │
...
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │  特点: 自动特征交叉，端到端学习                                       │    │
│  │  代表: Wide&Deep (2016), DeepFM (2017), DCN (2017)                   │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                   │                                         │
│                                   ▼                                         │
│  第三代: 用户行为序列建模                                                    │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │  特点: 建模用户兴趣演化，Attention 机制                               │    │
│  │  代表: DIN (2018), DIEN (2019), SIM (2020)                           │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                   │                                         │
│                                   ▼                                         │
│  第四代: 多目标 + 多场景                                                     │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │  特点: 同时优化多个目标，跨场景迁移                                   │    │
│  │  代表: ESMM (2018), MMOE (2018), PLE (2020)                          │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│                                                                              │
│  ┌───────────────────┐              ┌───────────────────────────────────┐   │
│  │  交叉特征          │              │  Sparse Features                   │   │
│  │  (人工设计)        │              │  (user_id, item_id, category, ...) │   │
│  │                   │              └───────────────┬───────────────────┘   │
│  │  例如:            │                              │                       │
│  │  user_age ×       │                              ▼                       │
│  │  item_category    │              ┌───────────────────────────────────┐   │
...
│       │        │        │              │                                    │
│       ▼        ▼        ▼              ▼                                    │
│  [emb_1]  [emb_2]  [emb_3]  ...   [emb_n]                                  │
│       │        │        │              │                                    │
│       │        │        │              │                                    │
│       └────────┴────────┴──────────────┘                                    │
│                         │                                                    │
│                         ▼ Attention with Target Item                        │
│              ┌──────────────────────┐                                       │
│              │  Attention Weights   │                                       │
│              │  a_i = f(emb_i, target_emb)                                  │
│              └──────────────────────┘                                       │
│                         │                                                    │
│                         ▼                                                    │
│              ┌──────────────────────┐                                       │
│              │  Weighted Sum        │                                       │
│              │  user_interest = Σ a_i × emb_i                               │
│              └──────────────────────┘                                       │
│                         │                                                    │
│                         ▼                                                    │
│              ┌──────────────────────┐                                       │
...
│  │                      用户特征 (User Features)                        │    │
│  │                                                                      │    │
│  │  • ID类: user_id, device_id                                          │    │
│  │  • 画像: 年龄, 性别, 地域, 职业                                       │    │
│  │  • 统计: 历史点击数, 购买数, 活跃天数                                 │    │
│  │  • 偏好: 类目偏好, 价格偏好, 品牌偏好                                 │    │
│  │  • 实时: 当前 Session 行为, 最近点击/购买                            │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │                      物品特征 (Item Features)                        │    │
│  │                                                                      │    │
│  │  • ID类: item_id, sku_id, shop_id, brand_id                          │    │
│  │  • 属性: 类目, 标签, 价格, 上架时间                                   │    │
│  │  • 统计: 曝光数, 点击数, CTR, 销量                                   │    │
│  │  • 内容: 标题 Embedding, 图片 Embedding                              │    │
│  │  • 实时: 实时点击数, 实时 CTR                                        │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
...
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │  • 热门召回: 推荐全局热门内容                                        │    │
│  │  • 设备特征: 根据设备类型、OS 版本推断用户画像                        │    │
│  │  • 渠道特征: 根据安装渠道推断用户兴趣                                │    │
│  │  • 地域特征: 根据 IP/GPS 推荐本地热门                                │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
│  阶段 2: 首次交互 (选择兴趣标签)                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
...
│                                                                              │
│  2. 文本/图像 Embedding                                                      │
│     • 使用预训练模型 (BERT, CLIP) 生成 Embedding                            │
│     • 与现有物品的 Embedding 计算相似度                                      │
│                                                                              │
│  3. 规则保量                                                                 │
│     • 新物品强制曝光一定次数                                                 │
│     • 收集初始行为数据                                                       │
...
│  4. 预计算                                                                   │
│     • 用户 Embedding 预计算                                                  │
│     • 物品 Embedding 预计算并建索引                                          │
│     • 协同过滤相似度离线计算                                                 │
│                                                                              │
│  5. 分层截断                                                                 │
│     • 召回 1000 → 粗排 200 → 精排 50 → 重排 20                              │
│     • 每层过滤，减少下层计算量                                               │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│  │                      分流服务                                        │    │
│  │                                                                      │    │
│  │  hash(user_id) % 100 → bucket                                       │    │
│  │                                                                      │    │
│  │  bucket 0-9:   实验 A (新模型)                                       │    │
│  │  bucket 10-19: 实验 B (新召回)                                       │    │
│  │  bucket 20-99: 对照组 (基线)                                         │    │
│  └───────────────────────────────────┬─────────────────────────────────┘    │
│                                      │                                       │
│         ┌────────────────────────────┼────────────────────────────┐         │
