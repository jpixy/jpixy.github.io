...
1. [背景与挑战](#一背景与挑战)
2. [消费模型基础](#二消费模型基础)
3. [多线程消费设计模式](#三多线程消费设计模式)
4. [线程模型深度设计](#四线程模型深度设计)
5. [消息顺序性保证](#五消息顺序性保证)
6. [幂等与去重](#六幂等与去重)
7. [位移管理与可靠性](#七位移管理与可靠性)
8. [高性能设计](#八高性能设计)
9. [高可用设计](#九高可用设计)
10. [自动伸缩](#十自动伸缩)
11. [不同 MQ 实现对比](#十一不同-mq-实现对比)
12. [生产环境最佳实践](#十二生产环境最佳实践)
13. [监控与告警](#十三监控与告警)
...
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │                      Task Queue (任务队列)                           │    │
│  │              BlockingQueue / Disruptor / Ring Buffer                 │    │
│  └───────┬───────────────┬───────────────┬───────────────┬─────────────┘    │
│          │               │               │               │                  │
│          ▼               ▼               ▼               ▼                  │
│  ┌───────────┐   ┌───────────┐   ┌───────────┐   ┌───────────┐             │
...
│  │                                                                      │    │
│  │  ┌───────────────┐ ┌───────────────┐ ┌───────────────┐ ┌───────────┐│    │
│  │  │  P0 Handler   │ │  P1 Handler   │ │  P2 Handler   │ │P3 Handler ││    │
│  │  │ ┌───────────┐ │ │ ┌───────────┐ │ │ ┌───────────┐ │ │┌─────────┐││    │
...
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │                          Partition                                   │    │
│  │   [order:1, A] [order:2, B] [order:1, C] [order:3, D] [order:2, E]  │    │
│  └────────────────────────────┬────────────────────────────────────────┘    │
│                               │                                             │
│                               ▼                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │                    Dispatcher (分发器)                               │    │
│  │                hash(businessKey) % workerCount                       │    │
│  └──────┬──────────────────┬──────────────────┬─────────────────────────┘    │
│         │                  │                  │                              │
│         ▼                  ▼                  ▼                              │
│  ┌─────────────┐    ┌─────────────┐    ┌─────────────┐                      │
...
│  │ Queue:      │    │ Queue:      │    │ Queue:      │                      │
│  │ order:1 [A,C]│   │ order:2 [B,E]│   │ order:3 [D] │                      │
│  │ (串行处理)  │    │ (串行处理)  │    │ (串行处理)  │                      │
│  └─────────────┘    └─────────────┘    └─────────────┘                      │
│                                                                              │
│  特点:                                                                       │
...
│  • session.timeout.ms: 会话超时 (默认 10 秒)                                │
│  • heartbeat.interval.ms: 心跳间隔 (默认 3 秒)                              │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│          ▼                       ▼                       ▼                 │
│  ┌───────────────┐       ┌───────────────┐       ┌───────────────┐        │
│  │PartitionWorker│       │PartitionWorker│       │PartitionWorker│        │
│  │     (P0)      │       │     (P1)      │       │     (P2)      │        │
│  │               │       │               │       │               │        │
│  │ 1. 从队列取任务│       │ 1. 从队列取任务│       │ 1. 从队列取任务│        │
│  │ 2. 执行业务逻辑│       │ 2. 执行业务逻辑│       │ 2. 执行业务逻辑│        │
│  │ 3. 标记完成   │       │ 3. 标记完成   │       │ 3. 标记完成   │        │
│  │ 4. 通知主线程 │       │ 4. 通知主线程 │       │ 4. 通知主线程 │        │
│  └───────────────┘       └───────────────┘       └───────────────┘        │
│          │                       │                       │                 │
│          └───────────────────────┼───────────────────────┘                 │
│                                  │                                          │
...
│  │  4. 提交事务                                                         │    │
│  │                                                                      │    │
│  │  实现: Kafka Transaction / 业务表记录 offset                         │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│                                                                              │
│  Kafka 配置:                                                                 │
│  • fetch.min.bytes: 最小拉取字节数 (默认 1)                                  │
│  • fetch.max.wait.ms: 最大等待时间 (默认 500ms)                              │
│  • max.partition.fetch.bytes: 每分区最大拉取 (默认 1MB)                      │
│  • max.poll.records: 单次最大记录数 (默认 500)                               │
│                                                                              │
│  优化建议:                                                                   │
...
│  异步处理:                                                                   │
│  ┌──────────────┐                                                           │
│  │  Poll Thread │ poll() → submit to queue → poll() → ...                  │
│  └──────────────┘                                                           │
│         │                                                                    │
│         ▼                                                                    │
│  ┌──────────────┐                                                           │
│  │ Worker Pool  │ process() → process() → ...                              │
│  └──────────────┘                                                           │
│                                                                              │
...
│  场景 1: 消费者进程崩溃                                                       │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │  1. Coordinator 检测到心跳超时 (session.timeout.ms)                  │    │
│  │  2. 触发 Rebalance                                                   │    │
│  │  3. 分区重新分配给其他消费者                                          │    │
│  │  4. 新消费者从上次提交的 offset 继续消费                              │    │
│  │                                                                      │    │
│  │  影响: 未提交的消息会重新消费                                         │    │
│  │  缓解: 减小提交间隔，幂等处理                                         │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
